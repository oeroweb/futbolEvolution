    ...
    $imagen_existente = isset($_POST['imagen_existente']) ? $_POST['imagen_existente'] : false;
		...
		if($nombre_imagen){
			if($tipo_imagen=="image/jpeg" || $tipo_imagen=="image/jpg" || $tipo_imagen=="image/png" || $tipo_imagen4=="image/svg"){
				...
				header("Location: ../../leaguecontacto-edit.php?id=$id");
				die();
			}		
		}else{
		...
		$sql="UPDATE ligascontacto SET en_titulo='$en_titulo', es_titulo='$es_titulo', en_descripcion='$en_descripcion', es_descripcion='$es_descripcion', imagen='$subir_imagen' WHERE id = $id;";
